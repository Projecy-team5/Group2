            ...
            @if ($scholarships->count() > 0)
                <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    @foreach ($scholarships as $scholarship)
                        ...
                        @if ($daysLeft >= 0)
                            <div
                                class="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-lg transition-shadow overflow-hidden flex flex-col h-full">
                                <div class="p-6">
                                ...
                                <div class="px-6 pb-6 flex-1 flex flex-col">
                                    <div class="flex items-center justify-between mb-4">
                                        ...
                                        <span
                                            class="text-2xl font-bold text-indigo-600">${{ $scholarship->award_amount }}</span>
                                    </div>
                                    ...
                                    <div class="mb-4 flex-1">
                                        <div class="flex items-center gap-2 text-sm text-gray-600 mb-2">
                                            ...
                                            <div class="d-flex justify-content-between">
                                                {{ $scholarship->country }}
                                                <span>Days left: {{ $deadline->format('M d, Y') }}</span>
                                            </div>
                                        </div>
                                        <p class="text-gray-600 text-sm line-clamp-3">
                                            {{ Str::limit($scholarship->application_description, 120) }}
                                        </p>
                                    </div>
                                    <a href="{{ route('scholarships.show', $scholarship) }}"
                                        class="w-full inline-flex items-center justify-center px-4 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-all font-medium">
                                        View Details
                                    </a>
                                </div>
                            </div>
