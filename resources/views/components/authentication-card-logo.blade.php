...
@php
    ...
    $computedAlt = $alt ?? $brandName . ' logo';
@endphp

<a href="/" class="inline-flex items-center">
    @if ($logoUrl)
        <img src="{{ $logoUrl }}" alt="{{ $computedAlt }}"
            {{ $attributes->class(['h-12 w-auto object-contain']) }}>
    @else
        <svg {{ $attributes->class(['size-16']) }} viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
