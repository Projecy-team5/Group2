...
@section('content')
    @php
        $selectedCategories = collect(old('categories', []))->map(fn($id) => (string) $id)->all();
        ...
        <div class="grid grid-cols-12 gap-6">
            <div class="col-span-12 rounded-2xl border border-gray-200 bg-white shadow-sm">
                <div class="flex items-center gap-3 border-b border-gray-100 px-6 py-4">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900">{{ __('Article Details') }}</h2>
                        <p class="text-sm text-gray-500">{{ __('Give the post a clear headline, synopsis, and the full story.') }}</p>
                    </div>
                </div>
                <form action="{{ route('admin.articles.store') }}" method="POST" class="px-6 py-6 space-y-8">
                    @csrf
                    <div class="space-y-6">
                                ...
                                <div class="mt-3 rounded-xl border border-gray-300 bg-gray-50 p-4">
                                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                        @foreach ($categories as $cat)
                                            <label class="flex items-center gap-3 cursor-pointer group/item hover:bg-white rounded-lg p-2 transition">
                                                ...
                                                <span class="text-sm font-medium text-gray-700 group-hover/item:text-gray-900">{{ $cat->name }}</span>
                                            </label>
                                        @endforeach
                                    </div>
                                </div>
                        ...
                        <div class="group">
                            ...
                            <textarea id="excerpt" name="excerpt" rows="3"
                                ...
                                placeholder="{{ __('Short description used for teasers') }}">{{ old('excerpt') }}</textarea>
                        </div>
                        <div class="group">
                            ...
                            <textarea id="content" name="content" rows="12" required
                                ...
                                placeholder="{{ __('Start writing your amazing article') }}">{{ old('content') }}</textarea>
                            @error('content')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="rounded-xl border border-gray-200 bg-gray-50 p-5">
                            <div class="flex items-center justify-between border-b border-gray-100 pb-4">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500">{{ __('Control when the article goes live.') }}</p>
                                </div>
                            </div>
