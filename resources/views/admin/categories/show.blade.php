        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 px-6 py-5">
                <div>
                    ...
                    <p class="text-sm text-slate-500">Category overview and recent articles.</p>
                </div>
            </div>
            <div class="px-6 py-6 space-y-6">
                <div class="grid gap-4 rounded-2xl border border-slate-100 bg-slate-50 p-4 text-sm text-slate-600 lg:grid-cols-4">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-wider text-slate-500">Slug</p>
                    ...
                    <div>
                        ...
                        <p class="mt-1 inline-flex items-center rounded-full bg-slate-100 px-3 py-1 text-sm font-semibold text-slate-800">
                            {{ $category->articles_count }} linked
                        </p>
                    </div>
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-wider text-slate-500">Created</p>
                        <p class="mt-1 text-base font-semibold text-slate-900">{{ $category->created_at->format('M d, Y') }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-wider text-slate-500">Last updated</p>
                        <p class="mt-1 text-base font-semibold text-slate-900">{{ $category->updated_at->format('M d, Y') }}</p>
                    </div>
                </div>
                ...
                <div class="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                    ...
                    <div class="mt-4">
                        @if ($category->articles->isEmpty())
                            <p class="text-sm text-slate-500">No articles have been tagged with this category yet.</p>
                        @else
                                    ...
                                    <li class="py-3">
                                        <div class="flex items-center justify-between">
                                            <div>
                                                ...
                                                <p class="text-xs text-slate-500">Published {{ $article->created_at->format('M d, Y') }}</p>
                                            </div>
                                            <a href="{{ route('admin.articles.edit', $article) }}"
                                                class="text-sm font-semibold text-sky-600 hover:text-sky-700">Edit</a>
                                        </div>
                                    </li>
                                @endforeach
                            </ul>
